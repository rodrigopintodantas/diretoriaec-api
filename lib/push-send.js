...
/**
 * Lembrete de cobrança para atletas pendentes.
 * @param {{ usuarioIds: number[], cobranca: { id: string, nome: string, nomeClube?: string } | null }} opts
 */
async function notificarCobrancaPendente(opts) {
  const { usuarioIds, cobranca } = opts;
  const nomeClube = (cobranca?.nomeClube || "clube").trim();
  const nomeCobranca = (cobranca?.nome || "Cobrança do clube").trim();
  const payload = {
    title: "Cobrança",
    body: `Você possui cobrança pendente no ${nomeClube}: ${nomeCobranca}.`,
    data: { cobrancaId: cobranca?.id ?? null, type: "cobranca_pendente" },
  };
  await Promise.all([enviarPayloadParaUsuarios(usuarioIds, payload), enviarPayloadNativoParaUsuarios(usuarioIds, payload)]);
}
